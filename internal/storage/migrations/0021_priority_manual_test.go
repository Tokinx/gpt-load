...
func TestPriorityManualMigrationAddsNullableColumnsAndPreservesRows(t *testing.T) {
	...
	if err := migrations.Up0021(db); err != nil {
		t.Fatalf("Up0021() error = %v", err)
	}
	if err := migrations.Validate0021(db); err != nil {
		t.Fatalf("Validate0021() error = %v", err)
	}
	...
	if err := migrations.Up0021(db); err != nil {
		t.Fatalf("second Up0021() error = %v", err)
	}
}
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err := migrations.ValidateRecoverable0021(db); err != nil {
				t.Fatalf("ValidateRecoverable0021() error = %v", err)
			}
			if err := migrations.Up0021(db); err != nil {
				t.Fatalf("Up0021() after partial schema error = %v", err)
			}
			if err := migrations.Validate0021(db); err != nil {
				t.Fatalf("Validate0021() after recovery error = %v", err)
			}
		})
	}
}
