...
const ID0021 = "0021_priority_manual"

type groupPriorityManual0021 struct {
	PriorityManual *int `gorm:"column:priority_manual"`
}

func (groupPriorityManual0021) TableName() string { return "groups" }

type credentialPriorityManual0021 struct {
	PriorityManual *int `gorm:"column:priority_manual"`
}

func (credentialPriorityManual0021) TableName() string { return "credentials" }

// Up0021 adds nullable priority_manual columns without changing existing rows.
func Up0021(db *gorm.DB) error {
	for _, definition := range []struct {
		model any
		name  string
	}{
		{model: &groupPriorityManual0021{}, name: "groups"},
		{model: &credentialPriorityManual0021{}, name: "credentials"},
	} {
...
func ValidateRecoverable0021(db *gorm.DB) error {
	for _, definition := range []struct {
		...
		{model: &groupPriorityManual0021{}, table: "groups"},
		{model: &credentialPriorityManual0021{}, table: "credentials"},
	} {
		...
		if db.Migrator().HasColumn(definition.model, "priority_manual") {
			if err := validatePriorityManualColumn0021(db, definition.table); err != nil {
				return err
			}
		}
	}
...
func Validate0021(db *gorm.DB) error {
	for _, table := range []string{"groups", "credentials"} {
		...
		if err := validatePriorityManualColumn0021(db, table); err != nil {
			return err
		}
	}
	return nil
...
func validatePriorityManualColumn0021(db *gorm.DB, table string) error {
